...
1. [Releases](https://github.com/wodus1201/ClaudeBattery/releases/latest)에서 `ClaudeBattery.zip` 다운로드
2. 압축을 풀고 `ClaudeBattery.app`을 **응용 프로그램** 폴더로 드래그
3. 더블클릭해서 실행 → 메뉴바 오른쪽에 클로드 HP 위젯이 뜹니다

**첫 실행 시 "확인되지 않은 개발자" 경고가 뜹니다.** 이 앱은 Apple 유료 개발자
인증서로 서명되어 있지 않기 때문입니다. 한 번만 이렇게 열어주세요:

> `ClaudeBattery.app`을 **우클릭 → 열기** → 대화상자에서 다시 **열기**
>
...
마지막으로 위젯 메뉴에서 **"로그인 시 자동 시작"**을 켜면 끝입니다.
...
```bash
git clone https://github.com/wodus1201/ClaudeBattery.git
cd ClaudeBattery
./install.sh
```
...
```bash
cd ~/ClaudeBattery && ./update.sh    # git pull → 재빌드 → 재시작
```
...
`/Applications/ClaudeBattery.app`을 휴지통으로 옮기면 됩니다.

(먼저 지워버렸다면 시스템 설정 → 일반 → 로그인 항목에서 ClaudeBattery를
...
universal 바이너리(arm64 + x86_64)로 빌드해 `ClaudeBattery.zip`을 만들고,
